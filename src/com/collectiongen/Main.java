...
/* Для себя проверка файлов в папке
File testfolder = new File("src/items/zhopa");
File[] testfolderlist = testfolder.listFiles();
System.out.println(testfolderlist.length);
*/
public class Main {
/*
    public HashMap<Integer, File> get_item(File folder_name, int percent){
        HashMap<Integer, File> tmpHashMap = new HashMap<Integer, File>();
        String pathToFolder = String.format("src/items/%s/%d/", folder_name, percent);
        ...
        tmpHashMap.put(percent, item_list[chosen_item]);
        return tmpHashMap;
    }
*/
    ...
    public static ArrayList<Object> get_item(File folder_name, int percent){
        ...
        if(folder_name.getName().equals("none")){
            String pathToFolder = String.format("%s/1.png", folder_name);
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            //Вот с этим бы разобраться, но мы типа создаем чета
            Graphics2D g2d = background.createGraphics();
            //Указываем цвет фона
            int redc = 0 + (int) (Math.random() * 256);
            int greenc = 0 + (int) (Math.random() * 256);
            int bluec = 0 + (int) (Math.random() * 256);
            g2d.setColor(new Color(redc, greenc, bluec));
            //Заполняем квадрат цветом
            g2d.fillRect(0, 0, background.getWidth(), background.getHeight());
            ...
            String outputPath = String.format("results/%d.png", i);
